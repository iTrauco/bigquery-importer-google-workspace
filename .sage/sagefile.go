...
import (
	...
	"go.einride.tech/sage/tools/sgko"
...
func All(ctx context.Context) error {
	...
	sg.SerialDeps(ctx, KoBuildLocal, GoModTidy, GitVerifyNoDiff)
	return nil
}
...
func KoBuildLocal(ctx context.Context) error {
	sg.Deps(ctx, sgko.PrepareCommand)
	sg.Logger(ctx).Printf("building local Docker image...")
	return sgko.Command(ctx, "build", "--local", "--bare", ".").Run()
}

func KoBuild(ctx context.Context, repo, tag string) error {
	sg.Deps(ctx, sgko.PrepareCommand)
	sg.Logger(ctx).Printf("building and publishing %s:%s...", repo, tag)
	cmd := sgko.Command(ctx, "build", "--bare", "--tags", tag, ".")
	cmd.Env = append(cmd.Env, fmt.Sprintf("KO_DOCKER_REPO=%s", repo))
	return cmd.Run()
}
